...
This is a big upgrade to our interactive computing tools because Traitlets is used everywhere in Jupyter/IPython.
...
We'll demonstrate the flexibility
of Traitlets by configuring the `autocall` traitlet/option of IPython from
the command line interface, configuration file, as well as observe effect of dynamically changing values.
...
create configuration files with help, as well as command line arguments parsing. The class's `autocall` traitlet
have `help=` and `default_value` strings that is tagged with `config=True`.
This informs the current application on how to automatically generate configuration files; decide of the option name
and document it. No need for the developer to sped time on deciding on a configuration
parameter name.
...
your default configuration file `~/.ipython/profile_default/ipython_config.py` referring to our `autocall` traitlet of
previous section:
...
Alternatively, Traitlets can also generate and parse command line arguments, so `ipython
...
Unlike many other libraries likes argparse or click, the listing of configuration options is decentralized and lives on
the object they configure, and there is no differences between adding a configuration option in the configuration file
or at the command line interface.
...
and Traitlets allow you to also arbitrarily configure `MySpawner` with
